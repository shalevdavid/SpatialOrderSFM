...
//// Shalev
//#include "retrieval/visual_index.h"    // Shalev - For vector<int>
...
void TwoViewGeometry::EstimateCalibrated(
  ...
  // Shalev start: for PAMI stop condition improvement
  int Ng = -1;
  if (PAMI_spatial_verification == 6 || PAMI_spatial_verification == 7)
  {
	  vector<int> X_features1(matches.size());
	  vector<int> X_features2(matches.size());
	  std::vector<Eigen::Vector2d> matched_points1(matches.size());
	  std::vector<Eigen::Vector2d> matched_points2(matches.size());
	  for (size_t i = 0; i < matches.size(); ++i) {
  ...
  const auto E_report = E_ransac.Estimate(matched_points1_N, matched_points2_N, Ng/*shalev*/);
  E = E_report.model;
  ...
  const auto F_report = F_ransac.Estimate(matched_points1, matched_points2, Ng/*shalev*/);
  F = F_report.model;
  ...
  const auto H_report = H_ransac.Estimate(matched_points1, matched_points2, Ng/*shalev*/);
  H = H_report.model;
...
bool TwoViewGeometry::DetectWatermark(
  ...
  const auto report = ransac.Estimate(inlier_points1, inlier_points2, -1/*shalev*/);
