...
void VisualIndex<kDescType, kDescDim, kEmbeddingDim>::Query(
  ...
  for (auto& image_score : *image_scores) {
	...
	// Shalev T.B.D
	cout << endl << "Query function: image_score.image_id = " << image_score.image_id << endl;

    // Finally, run verification for the current image.
    VoteAndVerifyOptions vote_and_verify_options;
    image_score.score += VoteAndVerify(vote_and_verify_options, options.num_images_after_verification, matches);

	// Shalev T.B.D
	cout << "Query function: image_score.score = " << image_score.score << endl;
  }
