...
// Shalev
#include "retrieval/vote_and_verify.h" //Shalev - For OverlapResult Class
extern int PAMI_spatial_verification; //Shalev
//extern int Ng_th; //Shalev
extern OverlapResult PAMI(vector<int> X_features1, vector<int> X_features2); //Shalev
...
void SiftCPUFeatureMatcher::Run() {
  ...
  while (true) {
    ...
    if (input_job.IsValid()) {
	  ...
	  // Shalev: The if part is added for PAMI
	  if (PAMI_spatial_verification == 4) {
		  ...
		  vector<int> X_features1(data.matches.size());
		  vector<int> X_features2(data.matches.size());
		  std::vector<Eigen::Vector2d> matched_points1(data.matches.size());
		  std::vector<Eigen::Vector2d> matched_points2(data.matches.size());
		  for (size_t i = 0; i < data.matches.size(); ++i) {
			  matched_points1[i] = points1[data.matches[i].point2D_idx1];
			  matched_points2[i] = points2[data.matches[i].point2D_idx2];
			  X_features1[i] = (int)matched_points1[i].x();
			  X_features2[i] = (int)matched_points2[i].x();
		  }

		  if (data.matches.size() > 0) {
			  OverlapResult overlapRes_PAMI = PAMI(X_features1, X_features2);

			  if ((int)overlapRes_PAMI.maxNg < 30) {
				  data.matches = {};
			  }
		  }
		  else {
...
void TwoViewGeometryVerifier::Run() {
  while (true) {
    ...
    if (input_job.IsValid()) {
	  ...
	  /// Shalev - Start
	  // Shalev - Handle PAMI for Exahustive feature matching possibility chosen by user.
	  if (PAMI_spatial_verification == 2 || PAMI_spatial_verification == 3 || PAMI_spatial_verification == 7) {
		  ...
		  //if ((int)overlapRes_PAMI.maxNg < 5000) {
			 // if (PAMI_spatial_verification == 3) {
				//  data.matches = {};
			 // }
			 // CHECK(output_queue_->Push(data));
			 // continue;
		  ...
		  if ( (int)overlapRes_PAMI.maxNg < static_cast<size_t>(options_.min_num_inliers)) {
			  if (PAMI_spatial_verification == 3) {
				  data.matches = {};
			  }
			  CHECK(output_queue_->Push(data));
	  ...
	  if (PAMI_spatial_verification == 5) {
		  CHECK(output_queue_->Push(data));
		  continue;
	  }
	  /// Shalev - End.
