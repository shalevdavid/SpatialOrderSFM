...
class LORANSAC : public RANSAC<Estimator, SupportMeasurer, Sampler> {
 public:
  ...
  Report Estimate(const std::vector<typename Estimator::X_t>& X,
				  ...
				  int Ng /* Shalev: adding Ng. if -1 then irrelevant*/);
...
LORANSAC<Estimator, LocalEstimator, SupportMeasurer, Sampler>::Estimate(
	...
	//int Ng = 0;
	//if (PAMI_spatial_verification == 6)
	//{
	//	vector<int> X_features1(X.size());
	//	vector<int> X_features2(Y.size());
	//	for (size_t i = 0; i < X.size(); ++i) {
	//		X_features1[i] = (int)X[i].x();
	//		X_features2[i] = (int)Y[i].x();
	//	}
  ...
  for (report.num_trials = 0; report.num_trials < max_num_trials;
    ...
    for (const auto& sample_model : sample_models) {
	  ...
	  // Shalev start :PAMI
	  if ( (Ng != -1) && (PAMI_spatial_verification == 6 || PAMI_spatial_verification == 7) ) {
		  if (best_support.num_inliers >= Ng) {
			  abort = true;
			  break;
		  }
	  }
